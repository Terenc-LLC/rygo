...
* **Hosting:** Vercel.
* **Serverless functions:** Vercel functions in `/api`, Node.js runtime, bundled by `@vercel/node`. First added in [TER-343](https://linear.app/terenc/issue/TER-343): `/api/og` renders dynamic share-card PNGs via `@vercel/og` (Satori). Distinct from the Supabase edge functions (Supabase-hosted). Validated on the Vercel deploy preview, not CI; `api/og.tsx` + `api/tsconfig.json` are excluded from `tsconfig.app.json`. Deps: `@vercel/og` (prod), `@fontsource/jetbrains-mono` (dev — source for the committed WOFF files).
* **Package manager:** npm
...
* **RefThumbnail** (`src/components/RefThumbnail.tsx`) — read-only minimap of the target board. `w-28` (112px), same color tokens as Grid, non-interactive divs. Always visible alongside the play Grid. Shipped in [TER-221](https://linear.app/terenc/issue/TER-221). Note: 8×8 at 112px → cells ≈12px — below the 15px shape-legibility threshold from design doc §8; flagged in TER-221 Linear comment for design decision. **Updated (June 5, 2026, [TER-338](https://linear.app/terenc/issue/TER-338)):** Light-mode Paper-gap alignment — `CELL_BG.empty` `bg-stone-300` → `bg-stone-400`; thumbnail button container and overlay board container `bg-grid-line` → `bg-paper dark:bg-ink`. The `--color-grid-line` token was the last consumer; removed from `src/index.css` `@theme` block. Dark mode unchanged. `RefThumbnail.test.tsx` assertions updated (`bg-grid-line` → `bg-paper`). RefThumbnail now matches the main play board's Paper-gap treatment in light mode.
...
### Share unfurl page — IN REVIEW (`api/s.ts`, `vercel.json`) [TER-344]
...
**Function (`api/s.ts`):** Plain TypeScript, Node.js runtime (no `export const config`), no JSX, no `@vercel/og`, no imports from `src/`. Reads `p` from `url.searchParams`. Builds absolute base URL from `x-forwarded-proto` + `x-forwarded-host`/`host` headers so preview and prod both resolve correctly. Tags: `og:title` = `RYGO — The daily color-logic puzzle`, `og:description` = homepage description, `og:url` = `/s/<payload>`, `og:image` = `/api/og?p=<payload>`, full `twitter:*` (`summary_large_image`), `<meta name="robots" content="noindex">`. No payload validation — invalid payloads pass through to `/api/og`, which renders the brand-only default card. Body: "Opening RYGO…" + `window.location.replace('/')`.

**`vercel.json` rewrite:** `{ "source": "/s/:payload", "destination": "/api/s?p=:payload" }` added above the existing `/tabs` rewrite. All other routes (`/tabs`, `/api/*`, `/`, static assets) are unaffected.

Shipped in [TER-344](https://linear.app/terenc/issue/TER-344), June 5, 2026.
...
* Tailwind v4 for all styling. CSS-based config (no `tailwind.config.js`). Brand tokens (`ink`, `paper`, `rygo-red`, `rygo-yellow`, `rygo-green`) defined via `@theme` block in `index.css` ([TER-152](https://linear.app/terenc/issue/TER-152); the `grid-line` token added in [TER-290](https://linear.app/terenc/issue/TER-290) was removed in [TER-338](https://linear.app/terenc/issue/TER-338) when its last consumer moved to `bg-paper`). No CSS modules.
...
* [TER-311](https://linear.app/terenc/issue/TER-311) — ✅ Done. Admin metrics dashboard at `/tabs` (Option B: anon aggregates RPC, no router): `get_admin_metrics()` RPC migration, `getAdminMetrics.ts` client reader, `AdminDashboard.tsx` component, `main.tsx` pathname branch, `vercel.json` rewrite. Unguarded by design. Filed June 4, 2026.
* [TER-313](https://linear.app/terenc/issue/TER-313) — ✅ Done. Screen fade transitions (CSS-only, reduced-motion-instant): `@keyframes screenFade` + `.screen-fade` in `index.css`, keyed `<div key={view}>` wrapper in `App.tsx`, Summary wrapper in `GameScreen.tsx`. Filed June 4, 2026.
* [TER-342](https://linear.app/terenc/issue/TER-342) — ✅ Done. Homepage SEO + social meta hardening: full `<head>` meta block in `index.html` (`<title>`, canonical, description, theme-color, og:type/site_name/title/description/image/width/height/url, twitter:card/title/description/image). Shipped June 5, 2026 (PR #91).
...
* [TER-322](https://linear.app/terenc/issue/TER-322) — ✅ Done. Privacy disclosure line on SettingsScreen: "Privacy" section heading + approved copy ("No account, no personal data. Your device gets a random ID to record daily leaderboard scores — that's all.") rendered below the toggles with `text-gray-500 dark:text-gray-400` secondary styling. Static, no toggle, no link-out. Test asserts heading and copy render. Filed June 4, 2026.
* [TER-332](https://linear.app/terenc/issue/TER-332) — ✅ Done. Light game board contrast pass: `CELL_BG.empty` `bg-stone-300` → `bg-stone-400`; grid container `bg-grid-line` → `bg-paper` (dividers gone, gaps show page color). `--color-grid-line` token retained (`RefThumbnail` still consumes it). `Grid.test.tsx` updated. Dark mode unchanged. Shipped June 4, 2026 (PR #86).
* [TER-338](https://linear.app/terenc/issue/TER-338) — ✅ Done. RefThumbnail light-mode Paper-gap alignment: `CELL_BG.empty` `bg-stone-300` → `bg-stone-400`; both container backgrounds `bg-grid-line` → `bg-paper`. `--color-grid-line` token removed from `index.css` (zero consumers after edit). `RefThumbnail.test.tsx` updated. Dark mode unchanged. Shipped June 5, 2026 (PR #89).
