...
**First Vercel serverless function in this repo** (separate from Supabase edge functions). `api/og.tsx` is not in `tsconfig.app.json` include — intentional; function is validated on the Vercel deploy preview, not CI. The shared codec (`src/share/resultCodec.ts`) IS covered by CI type-check and has 16 unit tests.
...
* [TER-343](https://linear.app/terenc/issue/TER-343) — ✅ In Review. Dynamic OG card renderer: `api/og.tsx` (Vercel Edge, `@vercel/og`/Satori, 1200×630 PNG), `src/share/resultCodec.ts` (base64url codec, 16 unit tests), `public/fonts/` WOFF font files. Part of the B-server unfurl pipeline.
